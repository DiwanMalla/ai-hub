...
interface ModelTryPanelProps {
  modelConfig: ModelConfig;
}

export default function ModelTryPanel({ modelConfig }: ModelTryPanelProps) {
  ...
  useEffect(() => {
    async function loadModel() {
      ...
      try {
        // Cast the dynamic task value to any to satisfy pipeline's expected PipelineType
        pipelineRef.current = await pipeline(
          modelConfig.pipeline.task as any,
          modelConfig.pipeline.model,
          {
    ...
    loadModel();
  }, [modelConfig.pipeline.task, modelConfig.pipeline.model]);
    ...
    try {
      ...
      async function maskToCanvas(
        ...
        if (
          ...
          maskObj instanceof ImageBitmap
        ) {
      ...
      if (!resizedMaskCtx)
        throw new Error("Could not get resized mask context");
