  ...
  const processImage = async () => {
    ...
    try {
      ...
      if (!pipelineRef.current) {
        throw new Error("AI model not loaded yet. Please wait for initialization.");
      }
      ...
      await new Promise((resolve, reject) => {
        img.onload = resolve;
        img.onerror = () => reject(new Error("Failed to load image. Please check the URL or try a different image."));
      ...
      try {
        ...
        result = await pipelineRef.current(selectedImage, { return_mask: true });
      } catch (pipelineError) {
        console.error("Pipeline execution error:", pipelineError);
        throw new Error("AI model processing failed. This might be due to browser compatibility or model loading issues.");
      }
      ...
      maskCanvas.width = (mask as any).width || canvas.width;
      maskCanvas.height = (mask as any).height || canvas.height;
      const maskCtx = maskCanvas.getContext("2d");
      if (!maskCtx) throw new Error("Could not get mask context");
      
      maskCtx.drawImage(mask as any, 0, 0);
      ...
      resizedMaskCtx.drawImage(mask as any, 0, 0, canvas.width, canvas.height);
