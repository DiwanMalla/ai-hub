...
const HUGGING_FACE_FIBO_ENDPOINT =
  "https://api-inference.huggingface.co/models/briaai/FIBO";
  ...
  try {
    ...
    if (!hfResponse.ok) {
      ...
      if (hfResponse.status === 401 || hfResponse.status === 403) {
        return NextResponse.json(
          {
            error:
              "Authentication failed. Verify your Hugging Face token has access to briaai/FIBO (gated model).",
            details: errorText,
          },
      ...
      if (hfResponse.status === 503) {
        return NextResponse.json(
          {
            error:
              "Model is loading. Please wait a few moments and try again.",
            details: errorText,
          },
          { status: 503 }
        );
    ...
    if (contentType?.includes("application/json")) {
      ...
      if (jsonResponse.image || jsonResponse[0]?.image) {
        ...
        const structuredPrompt = jsonResponse.json_prompt || jsonResponse[0]?.json_prompt;
